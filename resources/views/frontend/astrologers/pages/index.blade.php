    ...
    <div class="container">
        <div class="row p-3">
            <div class="col-12 mb-4">
                ...
                <div>
                    <p class="pt-3 text-center">Explore the mysteries of the zodiac, uncover your birth chart's secrets, and
                        navigate planetary alignments.</p>
                    <p class="text-center ">Delve into the intricate tapestry of astrology with personalized horoscopes,
                        insightful birth charts, and transformative guidance. Explore the mysteries of the stars and unlock
                        the secrets of your destiny.</p>
                </div>
            </div>
            <div class="col-sm-4">
                                ...
                                <form action="" id="chatForm">
                                    <li class="list-group-item d-flex justify-content-center align-items-center">
                                        ...
                                        <input type="hidden" name="userId" id="userId"
                                            value="{{ $request['astrologerId'] }}">
                                        <div class="d-flex justify-content-between">
                                            <div class="w-25 pr-1">
                                                @if ($request['profile'])
                                                    <img src="{{ $request['profile'] }}" class="rounded-circle img-fluid"
                                                        alt="Avatar">
                                                @else
                                            ...
                                            <div class="d-flex flex-column">
                                                ...
                                                <div class="d-flex">
                                                    <i class="fa fa-calendar mt-1"
                                                        aria-hidden="true"></i>&nbsp;<span>{{ date('d-m-Y', strtotime($request['chatcreatedat'])) }}</span>
                                                </div>
                                            </div>
                                            <div class="status">
                                                <a
                                                    class="bg-light-success text-dark border border-success px-3 py-2 acceptchat">Accept</a>
                                                <a
                                                    class="badge bg-light-danger text-dark border border-danger px-3 py-2 rejectchat">Reject</a>
                                            </div>
                                        </div>
                                    </li>
                                </form>
                                    ...
                                    <li class="list-group-item d-flex justify-content-center align-items-center">
                                        ...
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <div class="d-flex flex-column">
                                                ...
                                                <div class="d-flex">
                                                    <i class="fa fa-calendar mt-1"
                                                        aria-hidden="true"></i>&nbsp;<span>{{ date('d-m-Y', strtotime($request['callcreatedat'])) }}</span>
                                                </div>
                                            ...
                                            <div class="status">
                                                <a
                                                    class="badge bg-light-success text-dark border border-success px-3 py-2 acceptcall">Accept</a>
                                                <a
                                                    class="badge bg-light-danger text-dark border border-danger px-3 py-2 rejectcall">Reject</a>
                                            </div>
                                        </div>
                                    </li>
                                </form>
                    ...
                    <ul class="list-group list-group-flush astrorequests" id="reportRequests">
                        @if (isset($getUserReport['recordList']) && count($getUserReport['recordList']) > 0)
                            @foreach ($getUserReport['recordList'] as $getUserReport)
                                <div>
                                    <span
                                        class="text-dark font-weight-bold ml-2">{{ $getUserReport['reportType'] }}</span>
                                </div>
                                <li class="list-group-item d-flex justify-content-center align-items-center reportList"
                                    data-toggle="modal" data-target="#reportModal" data-id="{{ $getUserReport['id'] }}"
                                    style="cursor: pointer;">
                                    <div class="d-flex justify-content-between">
                                        <div class="w-25 pr-1">
                                            <img src="{{ $getUserReport['profile'] }}" class="rounded-circle img-fluid"
                                                alt="Avatar">
                                        </div>
                                        <div class="d-flex flex-column">
                                            <span>{{ $getUserReport['firstName'] }}
                                                {{ $getUserReport['lastName'] }}</span>
                                            <div class="d-flex">
                                                <i class="fa fa-calendar mt-1 "
                                                    aria-hidden="true"></i>&nbsp;<span>{{ date('d-m-Y', strtotime($getUserReport['birthDate'])) }}</span>
                                            </div>
                                            <div class="d-flex">
                                                <i
                                                    class="fa fa-clock mt-1"></i>&nbsp;<span>{{ $getUserReport['birthTime'] }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                ...
                success: function(response) {
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            window.location.href = "{{ route('front.astrologerchat') }}" +
                                "?partnerId=" +
                            ...
                            $.ajax({
                                ...
                                success: function(response_call) {
                                    ...
                                    window.location.href =
                                        "{{ route('front.astrologercall') }}" +
                                        "?callId=" + callId + "&call_type=" +
                                        call_type;
                                },
                                error: function(xhr, status, error) {
                                    var errorMessage = JSON.parse(xhr
                                        .responseText).error.paymentMethod[
                                        0];
            ...
            function updateChatRequests(requests) {
                ...
                if (requests && requests.length > 0) {
                    requests.forEach(request => {
                        const profileImg = request.profile ? `/${request.profile}` :
                            '{{ asset('public/frontend/astrowaycdn/dashaspeaks/web/content/images/blank-profile.png') }}';
                        chatRequests.append(`
                            <form id="chatForm">
                                <li class="list-group-item d-flex justify-content-center align-items-center">
                    ...
                    chatRequests.append(
                        '<li class="list-group-item d-flex justify-content-center align-items-center h-100"><p class="text-center card-title nodata">No Record Found !</p></li>'
                        );
                }
            }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status === 200 && response.recordList.length > 0) {
                            ...
                            var profileImage = record.profile ? record.profile :
                                '{{ asset('public/frontend/astrowaycdn/dashaspeaks/web/content/images/blank-profile.png') }}';
                            var html = `
                            <div>
                                <div class="user-card">
                            ...
                            console.log('Error: No record found or unexpected status ' +
                                response.status);
                        }
                    },
                    error: function(xhr) {

@php
    ...
    if (astroauthcheck()) {
        ...
        $chatrequest = DB::table('chatrequest')->where('userId', astroauthcheck()['id'])->get();
                                        ...
                                        @if (empty($astrologerId))
                                            <li class="nav-item">
                                                <div class="d-flex align-items-center justify-content-center">
                                                    <input type="hidden" name="astrologerId"
                                                        value="{{ astroauthcheck()['astrologerId'] }}">
                                                    <a class="btn btn-chat other-country goLiveBtn" id="goLiveBtn"><i
        ...
        fetch("{{ route('api.getUserNotification', ['token' => $token]) }}", {
            ...
            .then(data => {
                ...
                data.recordList.forEach(notification => {
                    ...
                    if (notification.id > lastchild && lastchild != undefined && (ChatStatus ||
                            CallStatus))

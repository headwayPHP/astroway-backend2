                    ...
                    <div id="{{ $group->flagGroupName }}"
                                ...
                                @if ($systemFlag->valueType == 'MultiSelectWebLang')
                                    ...
                                    <div>
                                        ...
                                        <select
                                            ...
                                            @foreach ($language as $lan)
                                                <option value="{{ $lan->languageCode }}"
                                                    {{ in_array($lan->languageCode, $selectedLanguages) ? 'selected' : '' }}>
                                                    {{ $lan->languageName }}
                                                </option>
                                            @endforeach
                                        </select>
                        ...
                        @if (count($group->subGroup) > 0)
                            @foreach ($group->subGroup as $subGroupIndex => $subGroup)
                                <h4
                                    class="my-4 text-lg font-medium {{ strtolower(str_replace(' ', '_', $subGroup->flagGroupName)) }}">
                                ...
                                @if ($subGroup->parentFlagGroupId == 2)
                                    <div class="mb-2">
                                        <input type="hidden" value="{{ $subGroup->id }}"
                                            name="flaggroups[{{ $subGroup->id }}][id]">
                                        <label>
                                            <input class="form-check-input" type="radio"
                                                name="flaggroups[{{ $subGroup->id }}][isActive]" value="1"
                                                {{ $subGroup->isActive ? 'checked' : '' }}>
                                            Enable
                                        </label>
                                        <label>
                                            <input class="form-check-input" type="radio"
                                                name="flaggroups[{{ $subGroup->id }}][isActive]" value="0"
                                                {{ !$subGroup->isActive ? 'checked' : '' }}>
                                            Disable
                                        </label>
                                    </div>
                                ...
                                <div class="box p-3 {{ strtolower(str_replace(' ', '_', $subGroup->flagGroupName)) }}">
                                    @if ($subGroup->flagGroupName == 'AstrologyAPI')
                                        <select name="astroApiCallType" id="astroApiCallType">
                                            ...
                                            <option value="3" {{ $astroApiCallType == 3 ? 'selected' : '' }}>Vedic
                                                Astro API</option>
                                        </select>
                                    @endif

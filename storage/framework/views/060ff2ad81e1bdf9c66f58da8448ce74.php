<div class="px_header_wrapper">
    <div class="px_header_detail">
        <!-- <div class="px_info_detail">
            <ul>
                <li>
                    <a href="javascript:;">
                        <div class="px_infobox">
                            <span class="px_infoicon">
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMid"
                                    width="18" height="19" viewBox="0 0 18 19">
                                    <defs>
                                        <style>
                                            .cls-1 {
                                                fill: var(--dark-color01);
                                                fill-rule: evenodd;
                                            }
                                        </style>
                                    </defs>
                                    <path
                                        d="M16.113,6.349 C15.971,2.828 12.839,0.003 9.009,0.003 C5.179,0.003 2.046,2.828 1.905,6.349 C0.710,6.580 0.006,7.429 0.006,8.663 L0.006,11.956 C0.006,13.720 1.386,14.346 2.569,14.346 L3.668,14.346 C4.036,14.346 4.337,14.060 4.337,13.708 L4.337,6.926 C4.337,6.574 4.036,6.288 3.668,6.288 L3.246,6.288 C3.409,3.489 5.931,1.259 9.009,1.259 C12.087,1.259 14.608,3.489 14.772,6.288 L14.350,6.288 C13.981,6.288 13.681,6.574 13.681,6.926 L13.681,13.698 C13.681,14.050 13.981,14.336 14.350,14.336 L14.761,14.336 C14.606,16.063 13.791,16.679 11.506,16.818 C11.468,16.304 11.013,15.897 10.460,15.897 L8.077,15.897 C7.500,15.897 7.029,16.340 7.029,16.886 L7.029,18.024 C7.029,18.569 7.500,19.013 8.077,19.013 L10.468,19.013 C11.020,19.013 11.473,18.608 11.513,18.095 C13.666,17.978 15.878,17.491 16.104,14.276 C17.304,14.048 18.012,13.197 18.012,11.960 L18.012,8.667 C18.012,7.431 17.307,6.579 16.113,6.349 ZM16.693,8.676 L16.673,11.956 C16.673,12.695 16.261,13.070 15.448,13.070 L15.019,13.070 L15.019,7.565 L15.467,7.565 C16.281,7.565 16.693,7.939 16.693,8.676 ZM2.550,7.540 L2.998,7.540 L2.998,13.059 L2.550,13.059 C1.737,13.059 1.325,12.684 1.325,11.946 L1.325,8.654 C1.325,7.915 1.737,7.540 2.550,7.540 ZM10.177,17.177 L10.177,17.740 L8.368,17.740 L8.368,17.177 L10.177,17.177 Z"
                                        class="cls-1" />
                                </svg>

                            </span>
                            <span class="px_orange">Talk to our Astrogers -</span>+1800 326 3264
                        </div>
                    </a>
                </li>
                <li>
                    <a href="javascript:;">
                        <div class="px_infobox">
                            <span class="px_infoicon">
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMid"
                                    width="18.032" height="18.031" viewBox="0 0 18.032 18.031">
                                    <defs>
                                        <style>
                                            .cls-1 {
                                                fill: var(--dark-color01);
                                                fill-rule: evenodd;
                                            }
                                        </style>
                                    </defs>
                                    <path
                                        d="M17.107,17.252 C17.081,17.280 17.065,17.293 17.049,17.304 C16.529,17.774 15.845,18.037 15.133,18.037 L14.664,18.037 C14.405,18.037 14.194,17.825 14.194,17.566 C14.194,17.306 14.405,17.095 14.664,17.095 L15.133,17.095 C15.449,17.095 15.759,17.018 16.038,16.870 L10.663,11.497 C9.749,10.581 8.260,10.581 7.345,11.497 L3.414,15.428 L7.754,15.428 C8.014,15.428 8.225,15.639 8.225,15.898 C8.225,16.158 8.014,16.369 7.754,16.369 L2.473,16.369 L1.971,16.870 C2.250,17.018 2.560,17.095 2.876,17.095 L11.572,17.095 C11.831,17.095 12.043,17.306 12.043,17.566 C12.043,17.825 11.831,18.037 11.572,18.037 L2.876,18.037 C1.285,18.037 -0.008,16.743 -0.008,15.152 L-0.008,6.244 C-0.008,6.120 0.040,6.003 0.129,5.913 L2.562,3.481 L2.562,1.524 C2.562,0.690 3.240,0.012 4.074,0.012 L13.935,0.012 C14.769,0.012 15.447,0.690 15.447,1.524 L15.447,3.499 L17.879,5.911 C17.967,6.000 18.017,6.121 18.017,6.244 L18.017,15.152 C18.017,15.954 17.694,16.700 17.107,17.252 ZM2.562,4.812 L1.128,6.244 L2.562,7.678 L2.562,4.812 ZM0.933,7.382 L0.933,15.152 C0.933,15.545 1.049,15.920 1.269,16.242 L5.531,11.979 L0.933,7.382 ZM14.506,1.524 C14.506,1.210 14.249,0.953 13.935,0.953 L4.074,0.953 C3.759,0.953 3.503,1.210 3.503,1.524 L3.503,8.619 L6.197,11.314 L6.679,10.831 C7.320,10.190 8.162,9.870 9.004,9.870 C9.846,9.870 10.688,10.190 11.330,10.831 L11.812,11.314 L14.506,8.619 L14.506,1.524 ZM15.447,4.824 L15.447,7.678 L16.880,6.245 L15.447,4.824 ZM17.075,7.382 L12.478,11.979 L16.740,16.242 C16.960,15.920 17.075,15.545 17.075,15.152 L17.075,7.382 ZM5.253,8.677 C5.253,8.417 5.465,8.206 5.724,8.206 L12.285,8.206 C12.544,8.206 12.755,8.417 12.755,8.677 C12.755,8.936 12.544,9.147 12.285,9.147 L5.724,9.147 C5.465,9.147 5.253,8.936 5.253,8.677 ZM12.285,6.648 L5.724,6.648 C5.465,6.648 5.253,6.436 5.253,6.177 C5.253,5.918 5.465,5.707 5.724,5.707 L12.285,5.707 C12.544,5.707 12.755,5.918 12.755,6.177 C12.755,6.436 12.544,6.648 12.285,6.648 ZM10.822,4.148 C10.698,4.148 10.578,4.098 10.489,4.011 C10.401,3.923 10.351,3.801 10.351,3.677 C10.351,3.551 10.401,3.430 10.489,3.344 C10.664,3.170 10.977,3.169 11.154,3.344 C11.242,3.430 11.292,3.551 11.292,3.677 C11.292,3.801 11.242,3.923 11.155,4.011 C11.066,4.098 10.945,4.148 10.822,4.148 ZM9.213,4.148 L5.724,4.148 C5.465,4.148 5.253,3.937 5.253,3.677 C5.253,3.418 5.465,3.206 5.724,3.206 L9.213,3.206 C9.472,3.206 9.683,3.418 9.683,3.677 C9.683,3.937 9.472,4.148 9.213,4.148 ZM13.119,17.095 L13.124,17.095 C13.383,17.095 13.595,17.306 13.595,17.566 C13.595,17.825 13.383,18.037 13.124,18.037 C12.860,18.037 12.648,17.825 12.648,17.566 C12.648,17.306 12.860,17.095 13.119,17.095 Z"
                                        class="cls-1" />
                                </svg>

                            </span>
                            <span class="px_orange">Talk to our Astrogers -</span>support@website.com
                        </div>
                    </a>
                </li>
            </ul>
            <div class="px_right_info">
                <a href="javascript:;">
                    <div class="px_infobox" data-bs-toggle="modal" data-bs-target="#px_login">
                        <span class="px_infoicon">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                preserveAspectRatio="xMidYMid" width="18" height="18" viewBox="0 0 18 18">
                                <defs>
                                    <style>
                                        .cls-1 {
                                            fill: var(--dark-color01);
                                            fill-rule: evenodd;
                                        }
                                    </style>
                                </defs>
                                <path
                                    d="M17.250,14.249 L14.250,14.249 L14.250,17.249 C14.250,17.664 13.915,17.999 13.500,17.999 C13.085,17.999 12.750,17.664 12.750,17.249 L12.750,14.249 L9.750,14.249 C9.335,14.249 9.000,13.914 9.000,13.499 C9.000,13.085 9.335,12.749 9.750,12.749 L12.750,12.749 L12.750,9.750 C12.750,9.336 13.085,8.999 13.500,8.999 C13.915,8.999 14.250,9.336 14.250,9.750 L14.250,12.749 L17.250,12.749 C17.665,12.749 18.000,13.085 18.000,13.499 C18.000,13.914 17.665,14.249 17.250,14.249 ZM7.500,10.499 C6.505,10.499 5.588,10.171 4.842,9.622 C2.863,10.604 1.500,12.645 1.500,14.999 L1.500,15.749 C1.500,16.163 1.836,16.499 2.250,16.499 L9.000,16.499 C9.415,16.499 9.750,16.836 9.750,17.249 C9.750,17.664 9.415,17.999 9.000,17.999 L2.250,17.999 C1.009,17.999 -0.000,16.990 -0.000,15.749 L-0.000,14.999 C-0.000,12.224 1.519,9.805 3.766,8.509 C3.283,7.791 3.000,6.928 3.000,5.999 L3.000,4.499 C3.000,2.018 5.018,-0.001 7.500,-0.001 C9.982,-0.001 12.000,2.018 12.000,4.499 L12.000,5.999 C12.000,8.481 9.982,10.499 7.500,10.499 ZM10.500,4.499 C10.500,2.845 9.154,1.499 7.500,1.499 C5.845,1.499 4.500,2.845 4.500,4.499 L4.500,5.999 C4.500,7.653 5.845,8.999 7.500,8.999 C9.154,8.999 10.500,7.653 10.500,5.999 L10.500,4.499 Z"
                                    class="cls-1" />
                            </svg>

                        </span>
                        <span class="px_logintext">Log in / Register</span>
                    </div>
                </a>
                <div class="px_cart">
                    <div class="px_cart_wrapper">
                        <span>
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                preserveAspectRatio="xMidYMid" width="18" height="18" viewBox="0 0 18 18">
                                <defs>
                                    <style>
                                        .cls-1 {
                                            fill: var(--dark-color01);
                                            fill-rule: evenodd;
                                        }
                                    </style>
                                </defs>
                                <path
                                    d="M18.000,6.047 L18.000,8.858 C18.000,9.247 17.685,9.561 17.297,9.561 L16.834,9.561 L16.538,12.198 C16.498,12.557 16.193,12.823 15.840,12.823 C15.814,12.823 15.787,12.822 15.761,12.818 C15.375,12.775 15.097,12.427 15.141,12.041 L15.507,8.780 C15.547,8.425 15.848,8.156 16.206,8.156 L16.594,8.156 L16.594,6.749 L1.406,6.749 L1.406,8.156 L13.394,8.156 C13.783,8.156 14.097,8.470 14.097,8.858 C14.097,9.247 13.783,9.561 13.394,9.561 L2.594,9.561 L3.288,15.354 C3.372,16.060 3.973,16.593 4.684,16.593 L13.372,16.593 C14.089,16.593 14.689,16.056 14.770,15.343 C14.813,14.958 15.161,14.680 15.547,14.723 C15.933,14.766 16.210,15.114 16.167,15.501 C16.007,16.925 14.805,17.999 13.372,17.999 L4.684,17.999 C3.261,17.999 2.061,16.933 1.891,15.521 L1.177,9.561 L0.703,9.561 C0.315,9.561 -0.000,9.247 -0.000,8.858 L-0.000,6.047 C-0.000,5.658 0.315,5.344 0.703,5.344 L3.197,5.344 L6.912,0.287 C7.142,-0.026 7.582,-0.094 7.895,0.136 C8.208,0.366 8.276,0.807 8.045,1.119 L4.942,5.344 L13.084,5.344 L9.980,1.119 C9.750,0.807 9.817,0.366 10.130,0.136 C10.443,-0.094 10.883,-0.027 11.113,0.287 L14.829,5.344 L17.297,5.344 C17.685,5.344 18.000,5.658 18.000,6.047 L18.000,6.047 ZM8.297,11.671 L8.297,14.484 C8.297,14.871 8.612,15.186 9.000,15.186 C9.388,15.186 9.703,14.871 9.703,14.484 L9.703,11.671 C9.703,11.283 9.388,10.968 9.000,10.968 C8.612,10.968 8.297,11.283 8.297,11.671 L8.297,11.671 ZM11.109,11.671 L11.109,14.484 C11.109,14.871 11.424,15.186 11.812,15.186 C12.201,15.186 12.516,14.871 12.516,14.484 L12.516,11.671 C12.516,11.283 12.201,10.968 11.812,10.968 C11.424,10.968 11.109,11.283 11.109,11.671 L11.109,11.671 ZM5.484,11.671 L5.484,14.484 C5.484,14.871 5.799,15.186 6.187,15.186 C6.576,15.186 6.890,14.871 6.890,14.484 L6.890,11.671 C6.890,11.283 6.576,10.968 6.187,10.968 C5.799,10.968 5.484,11.283 5.484,11.671 L5.484,11.671 Z"
                                    class="cls-1" />
                            </svg>

                            <span class="px_cartnumber">02</span></span>
                        $512
                    </div>

                    <div class="px_cart_box">
                        <div class="px_cart_list">
                            <ul>
                                <li>
                                    <div class="px_cart_img">
                                        <img src="public/images/prod2.jpg" class="img-responsive" alt="">
                                    </div>
                                    <div class="px_cart_info">
                                        <a href="#">Yellow Sapphire</a>
                                        <p>1 X $20.00</p>
                                        <a href="javascript:;" class="px_cart_remove"><i
                                                class="fa fa-trash"></i></a>
                                    </div>
                                </li>
                                <li>
                                    <div class="px_cart_img">
                                        <img src="public/images/prod4.jpg" class="img-responsive" alt="">
                                    </div>
                                    <div class="px_cart_info">
                                        <a href="#">yantra</a>
                                        <p>1 X $10.00</p>
                                        <a href="javascript:;" class="px_cart_remove"><i
                                                class="fa fa-trash"></i></a>
                                    </div>
                                </li>
                            </ul>
                        </div>
                        <div class="px_cart_total">
                            <p>total<span>$30.00</span></p>
                        </div>
                        <div class="px_cart_btn">
                            <button type="button" class="px_btn">view cart</button>
                            <button type="button" class="px_btn">checkout</button>
                        </div>
                    </div>
                </div>
            </div>
        </div> -->
        <div class="px_menu_wrapper" style="background-color: rgb(143, 26, 26);">
            <span class="px_toggle bg_overlay">
                <img src="public/images/svg/menu.svg" alt="">
            </span>
            <div class="px_logo">
                <!-- <a href="javascript:;"> -->
                <!-- <svg xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1"   viewBox="0 0 167.25 50" width="200px" height="60px"> -->
                <img src="public/images/logo.png" alt="logo" srcset="" width="100"
                    style="margin-right: 1em; width: 3.1em;">
                <h1>Dhaval Zaveri</h1>
                <!-- <path fill="#ffffff"
                            d="M65.35,16H60a1.82,1.82,0,0,0-.66.11.43.43,0,0,0-.28.44c.26.42.52.84.77,1.27a6.22,6.22,0,0,1,.34,2.47V29a13.41,13.41,0,0,1-.64,4.15,9,9,0,0,1-2,3.44c0,.06-.06.19-.13.39a.34.34,0,0,0,.11.25.52.52,0,0,0,.24.08,1.1,1.1,0,0,0,.4-.1,1.64,1.64,0,0,1,.32-.09,16.29,16.29,0,0,0,4.37-3.36A7.14,7.14,0,0,0,65,29V20.27c0-1.58.26-2.61.8-3.11a.91.91,0,0,0,.35-.6A.59.59,0,0,0,66,16.1,1.05,1.05,0,0,0,65.35,16Z">
                        </path>
                        <path fill="#ffffff"
                            d="M83.92,16.09a9.26,9.26,0,0,0-1.6-.11l-1,0H80.4l-.33.19a.91.91,0,0,0-.2.66,2.33,2.33,0,0,1,.53,2.31l-1.27,3.08c-.5,1.15-.95,2.15-1.38,3,0,0-.16,0-.38-.11L75,19.28a1.36,1.36,0,0,1-.13-.61,2.39,2.39,0,0,1,.31-1,2.5,2.5,0,0,0,.32-1c0-.43,0-.66-.14-.72H69.23l-.64,0a2,2,0,0,0-.7.1.55.55,0,0,0-.35.36,1.16,1.16,0,0,0-.1.34l.16.83q1.19,1.92,2.4,3.85,1.32,2.18,2.17,3.93a9.8,9.8,0,0,1,.88,2.56,14.83,14.83,0,0,1,.28,2.86,15.18,15.18,0,0,1-.2,2.53,2.8,2.8,0,0,1-.57.69.91.91,0,0,0-.33.79c.12.3.3.44.52.44h6.41a.36.36,0,0,0,.35-.2.78.78,0,0,0,.11-.4.52.52,0,0,0-.08-.3,4.17,4.17,0,0,1-.75-1,2,2,0,0,1-.27-1,13.63,13.63,0,0,1-.08-1.41,16.43,16.43,0,0,1,2-7.34c1.27-2.2,2.54-4.4,3.83-6.6a1.11,1.11,0,0,0,0-.28A.67.67,0,0,0,83.92,16.09Z">
                        </path>
                        <path fill="#ffffff"
                            d="M101.58,17.3C100.29,16.4,98,16,94.81,16s-5.39.48-6.71,1.43q-2.59,1.88-2.59,7a12.23,12.23,0,0,0,2.12,7,8.21,8.21,0,0,0,7.18,3.87,8,8,0,0,0,7-3.82,13.47,13.47,0,0,0,2.2-7.67C104,20.55,103.19,18.42,101.58,17.3Zm-4,12.38C97,32.19,96,33.44,94.75,33.44c-1.45,0-2.46-1.25-3-3.76a23.79,23.79,0,0,1-.5-5.29,11.27,11.27,0,0,1,.93-5,2.9,2.9,0,0,1,2.71-1.83c1.5,0,2.51,1,3,2.89a17.4,17.4,0,0,1,.36,4.15A21.36,21.36,0,0,1,97.59,29.68Z">
                        </path>
                        <path fill="#ffffff"
                            d="M120.09,16.2H107.3c-.66,0-1.2.51-1.63,1.54a6.27,6.27,0,0,0-.49,2.39,5.42,5.42,0,0,0,.09,1.14.47.47,0,0,0,.49.4c.31,0,.8-.53,1.47-1.61a3.07,3.07,0,0,1,2.71-1.61,1.39,1.39,0,0,1,.78.43,1.22,1.22,0,0,1,.48.92l0,11.22a4.55,4.55,0,0,1-.58,2.56,2.84,2.84,0,0,0-.58,1.21c0,.28.28.41.83.41h5.64a1.43,1.43,0,0,0,.55-.16c.24-.11.36-.26.36-.44-.32-.53-.61-1.07-.88-1.62a4.15,4.15,0,0,1-.42-1.9V19.8a1.9,1.9,0,0,1,.25-1,.81.81,0,0,1,.74-.4,3.25,3.25,0,0,1,2.45,1,14.11,14.11,0,0,1,1.73,2.23.43.43,0,0,0,.44-.34,2.57,2.57,0,0,0,.12-.84v-.36a7.05,7.05,0,0,0-.53-2.63C121,16.63,120.58,16.2,120.09,16.2Z">
                        </path>
                        <path fill="#ffffff"
                            d="M129.3,30.5l0-10.75a3.4,3.4,0,0,1,.56-2,2.48,2.48,0,0,0,.57-1,.61.61,0,0,0-.28-.54,1.27,1.27,0,0,0-.71-.18h-5.34a1.57,1.57,0,0,0-.87.15.56.56,0,0,0-.17.46l.74,1.4a3.85,3.85,0,0,1,.36,1.79v10.7a6.9,6.9,0,0,1-.36,2.49,10.85,10.85,0,0,1-.8,1.6c0,.17.12.32.36.46a1.19,1.19,0,0,0,.55.2h5.72a1.14,1.14,0,0,0,.54-.17.51.51,0,0,0,.31-.43,1.64,1.64,0,0,0-.38-.88A5.37,5.37,0,0,1,129.3,30.5Z">
                        </path>
                        <path fill="#ffffff"
                            d="M144,24.53c-1.68-1.23-3.36-2.46-5-3.71A2.29,2.29,0,0,1,138,19.2a1.23,1.23,0,0,1,.14-.57,2.23,2.23,0,0,1,.58-.62,3.8,3.8,0,0,1,1.76-.41,4.62,4.62,0,0,1,1.47.22,3.6,3.6,0,0,1,1.22.69,4.38,4.38,0,0,1,.91,1,1.08,1.08,0,0,0,1.05.52.75.75,0,0,0,.41-.63,2,2,0,0,1,0-.3,7.42,7.42,0,0,0-.25-1.54c-.06-.22-.12-.49-.19-.82a1.23,1.23,0,0,0-.29-.63.73.73,0,0,0-.48-.15,3.58,3.58,0,0,0-.62.1c-.32.06-.59.11-.8.13a10.29,10.29,0,0,1-1.23.05c-.34,0-1,0-2-.1S138,16,137.55,16a10.64,10.64,0,0,0-2.2.22,5.18,5.18,0,0,0-1.78.77A3.46,3.46,0,0,0,132,20a2.07,2.07,0,0,0,0,.41A4.73,4.73,0,0,0,133.37,23a10.44,10.44,0,0,0,2.37,1.9c1.08.72,2.16,1.42,3.24,2.12a5.23,5.23,0,0,1,2.26,3,2.93,2.93,0,0,1,0,.49,3.31,3.31,0,0,1-.32,1.42A2.63,2.63,0,0,1,140,33a2.14,2.14,0,0,1-.62.34,1.87,1.87,0,0,1-.64.1,2.88,2.88,0,0,1-.61-.09c-.26,0-.48-.11-.66-.16a2,2,0,0,1-1.46-2,5.9,5.9,0,0,0,.17-.66,1.6,1.6,0,0,1,.38-.83,2.24,2.24,0,0,1,.8-.47q.66-.27.66-.66a2,2,0,0,0-.84-.56,3.31,3.31,0,0,0-1.11-.18,5.9,5.9,0,0,0-1,.07,6.62,6.62,0,0,0-1,.23,3.79,3.79,0,0,0-1.45,1.12,2.47,2.47,0,0,0-.62,1.58,4.21,4.21,0,0,0-.05.52,2.7,2.7,0,0,0,.47,1.56,4.24,4.24,0,0,0,1.23,1.19,12.89,12.89,0,0,0,5.48,1.1,15.24,15.24,0,0,0,4.54-.66,4,4,0,0,0,1.88-1.33,4.75,4.75,0,0,0,1-2.13c.06-.33.1-.57.11-.71a4.27,4.27,0,0,0,0-.59Q146.74,27.07,144,24.53Z">
                        </path>
                        <path fill="#ffffff"
                            d="M164.9,33.31a9.86,9.86,0,0,1-.31-3V20.52q0-2.6.69-3.36a.89.89,0,0,0,.28-.49q0-.72-1.41-.72h-4.59c-.73,0-1.1.23-1.1.69a3.17,3.17,0,0,0,.48.93,2.8,2.8,0,0,1,.48,1.46l.06,1.29q0,.36-.57.54a13.85,13.85,0,0,1-2.13.37,8.41,8.41,0,0,0-1.24.33,3.16,3.16,0,0,1-.85.19c-.24,0-.36-.15-.36-.46V19a2,2,0,0,1,.42-1.32,1.61,1.61,0,0,0,.41-.94c0-.29-.06-.48-.17-.56a.89.89,0,0,0-.52-.12h-5.58a.73.73,0,0,0-.5.13.55.55,0,0,0-.25.47c.28.59.56,1.16.86,1.73a26.59,26.59,0,0,1,.35,5.2V29a15.46,15.46,0,0,1-.35,3.86,4.07,4.07,0,0,1-.86,1.72.5.5,0,0,0,.36.5,3.16,3.16,0,0,0,1.05.13h4.84a1,1,0,0,0,.51-.15.46.46,0,0,0,.23-.4c-.22-.45-.43-.91-.63-1.37a6.52,6.52,0,0,1-.28-2.17v-3.8a4.11,4.11,0,0,1,1.9-3.3A5,5,0,0,1,159,22.8c.34,0,.52.1.52.3v8.75a3.13,3.13,0,0,1-.48,1.87,2,2,0,0,0-.48,1,.35.35,0,0,0,.23.35,2.32,2.32,0,0,0,.76.09h5.14l.55-.08a.41.41,0,0,0,.33-.41C165.37,34.25,165.13,33.78,164.9,33.31Z">
                        </path>
                        <path fill="#ffffff"
                            d="M10,11.19a9.79,9.79,0,0,0,.84,3c.3.55.55.8,1,.12a14.1,14.1,0,0,1,3.31-3.23c.59-.41.48-.73.08-1.19-1.8-2.12-4.13-2.68-7.12-2.42A5.32,5.32,0,0,1,10,11.19Z">
                        </path>
                        <path fill="#ffffff"
                            d="M23.37,7.76a15.56,15.56,0,0,1,4.72,0c.67.1.79-.21.87-.75A7.07,7.07,0,0,0,25.62.49c.22,1.58-.63,2.71-1.41,3.85A8.86,8.86,0,0,0,22.76,7C22.6,7.52,22.63,7.87,23.37,7.76Z">
                        </path>
                        <path fill="#ffffff"
                            d="M46.23,23.67a9.47,9.47,0,0,0-2.73-1.51c-.56-.16-.77-.06-.7.6a19.21,19.21,0,0,1-.06,4.85c-.06.46,0,.63.5.73A6.81,6.81,0,0,0,50,25.12,5.77,5.77,0,0,1,46.23,23.67Z">
                        </path>
                        <path fill="#ffffff"
                            d="M36.2,11.27a14.31,14.31,0,0,1,3.3,3.38c.39.56.68.41,1.1.07,2.18-1.79,2.74-4.14,2.44-7-1.24,1.38-2.78,1.57-4.29,1.83a7.41,7.41,0,0,0-2.46.76C35.76,10.58,35.6,10.83,36.2,11.27Z">
                        </path>
                        <path fill="#ffffff"
                            d="M8.26,27.17a16,16,0,0,1,0-4.73c.1-.66-.22-.78-.75-.86A7.07,7.07,0,0,0,1,24.92c1.58-.22,2.7.63,3.85,1.41a8.86,8.86,0,0,0,2.62,1.45C8,27.94,8.38,27.91,8.26,27.17Z">
                        </path>
                        <path fill="#ffffff"
                            d="M14.91,38.75a15.74,15.74,0,0,1-3.27-3.4c-.39-.55-.71-.42-1.14-.09a7,7,0,0,0-2.35,7c1-1.26,2.37-1.44,3.74-1.68a9.21,9.21,0,0,0,2.89-.79C15.28,39.47,15.51,39.2,14.91,38.75Z">
                        </path>
                        <path fill="#ffffff"
                            d="M27.77,42.27a15.56,15.56,0,0,1-4.72-.1c-.66-.1-.79.21-.87.75a7,7,0,0,0,3.26,6.59c-.21-1.59.65-2.7,1.45-3.83a9.15,9.15,0,0,0,1.49-2.61C28.54,42.52,28.51,42.16,27.77,42.27Z">
                        </path>
                        <path fill="#ffffff"
                            d="M39.28,35.61a15.51,15.51,0,0,1-3.4,3.27c-.55.39-.42.71-.09,1.14a7,7,0,0,0,7,2.36c-1.26-1-1.44-2.37-1.68-3.74a9.06,9.06,0,0,0-.79-2.89C40,35.25,39.73,35,39.28,35.61Z">
                        </path>
                        <path fill="#ffffff"
                            d="M30.75,22.42l-3.48-.26-2-2.89-2,2.89-3.48.26,1.52,3.14-1.52,3.15,3.48.26,2,2.88,2-2.88,3.48-.26-1.51-3.15ZM27.4,26.78l-1.34.1L25.31,28l-.76-1.11-1.34-.1.58-1.22-.58-1.21,1.34-.1.76-1.11.75,1.11,1.34.1-.58,1.21Z">
                        </path>
                        <path fill="#ffffff"
                            d="M25.52,10.35a14.72,14.72,0,0,0-4.28.63c0,.09,0,.19,0,.28A3.71,3.71,0,0,1,17.54,15a3.65,3.65,0,0,1-2.11-.66,14.84,14.84,0,0,0,2.24,23.48,2.08,2.08,0,0,1,4,.74,2.05,2.05,0,0,1-.23.95,15.06,15.06,0,0,0,4.05.56,14.85,14.85,0,1,0,0-29.69Zm-.21,25a9.8,9.8,0,1,1,9.8-9.8A9.8,9.8,0,0,1,25.31,35.36Z">
                        </path> -->
                <!-- </svg> -->
                <!-- </a> -->
            </div>

            <div class="px_menu">
                <ul>
                    <li><a href="<?php echo e(route('front.home')); ?>">home</a>
                        <!-- <ul class="px_submenu">
                    <li><a href="index-2.html">home one</a></li>
                    <li><a href="index-light.html">home two</a></li>
                </ul> -->
                    </li>
                    <li><a href="<?php echo e(route('front.aboutus')); ?>">about</a></li>
                    <li><a href="<?php echo e(route('front.services')); ?>">services</a></li>
                    <!--<li><a href="https://www.instagram.com/dhavalzaverivaastu/">blog</a> -->
                    <!-- <ul class="px_submenu">
                            <li><a href="blog.html">blog</a></li>
                            <li><a href="javascript:;">blog single</a>
                                <ul class="px_submenu">
                                    <li><a href="blog_left_detail.html">left sidebar</a></li>
                                    <li><a href="https://www.instagram.com/dhavalzaverivaastu/">right sidebar</a></li>
                                </ul>
                            </li>
                        </ul> -->
                    </li>
                    <li><a href="<?php echo e(route('front.remotebooking')); ?>">remote booking</a></li>
                    <li><a href="<?php echo e(route('front.appointment')); ?>">appointment</a></li>
                    <!-- <li><a href="shop.html">shop</a> -->
                    <!-- <ul class="px_submenu">
                            <li><a href="shop.html">Shop</a></li>
                            <li><a href="shop_single.html">Shop Single</a></li>
                            <li><a href="cart.html">Cart</a></li>
                            <li><a href="checkout.html">Checkout</a></li>
                        </ul> -->
                    <!-- </li> -->
                    <!-- <li><a href="javascript:;">pages</a>
                        <ul class="px_submenu">
                            <li><a href="astrologer.html">Astrologer</a></li>
                            <li><a href="privacy_policy.html">Privacy Policy</a></li>
                            <li><a href="error.html">404</a></li>
                            <li><a href="faq.html">faq</a></li>
                        </ul>
                    </li> -->
                    <li><a href="<?php echo e(route('front.contact')); ?>">contact</a></li>
                </ul>
                <!-- <div class="px_search_wrapper">
                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                        preserveAspectRatio="xMidYMid" width="15" height="16.062" viewBox="0 0 15 16.062">
                        <path
                            d="M12.490,7.043 C12.466,7.867 12.244,8.963 11.679,9.945 C11.478,10.293 11.538,10.480 11.794,10.743 C12.735,11.714 13.658,12.704 14.576,13.701 C15.305,14.492 15.040,15.680 14.075,15.977 C13.544,16.140 13.121,15.922 12.750,15.522 C11.839,14.541 10.915,13.574 10.010,12.587 C9.794,12.351 9.642,12.287 9.335,12.466 C7.170,13.733 4.989,13.712 2.905,12.309 C0.866,10.936 -0.084,8.872 0.001,6.307 C0.104,3.199 2.407,0.556 5.349,0.100 C9.133,-0.485 12.508,2.645 12.490,7.043 ZM10.617,6.671 C10.611,4.126 8.643,2.021 6.270,2.019 C3.829,2.018 1.865,4.109 1.865,6.707 C1.864,9.265 3.848,11.365 6.257,11.359 C8.657,11.354 10.623,9.240 10.617,6.671 Z" />
                    </svg>

                    <div class="px_search_boxpopup">
                        <a href="javascript:;" class="px_cancel"><svg version="1.1"
                                xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                width="25px" height="25px" x="0px" y="0px" viewBox="0 0 511.76 511.76"
                                style="enable-background:new 0 0 511.76 511.76;" xml:space="preserve">
                                <g>
                                    <g>
                                        <path
                                            d="M436.896,74.869c-99.84-99.819-262.208-99.819-362.048,0c-99.797,99.819-99.797,262.229,0,362.048 c49.92,49.899,115.477,74.837,181.035,74.837s131.093-24.939,181.013-74.837C536.715,337.099,536.715,174.688,436.896,74.869z M361.461,331.317c8.341,8.341,8.341,21.824,0,30.165c-4.16,4.16-9.621,6.251-15.083,6.251c-5.461,0-10.923-2.091-15.083-6.251 l-75.413-75.435l-75.392,75.413c-4.181,4.16-9.643,6.251-15.083,6.251c-5.461,0-10.923-2.091-15.083-6.251 c-8.341-8.341-8.341-21.845,0-30.165l75.392-75.413l-75.413-75.413c-8.341-8.341-8.341-21.845,0-30.165 c8.32-8.341,21.824-8.341,30.165,0l75.413,75.413l75.413-75.413c8.341-8.341,21.824-8.341,30.165,0 c8.341,8.32,8.341,21.824,0,30.165l-75.413,75.413L361.461,331.317z" />
                                    </g>
                                </g>
                                <g> </g>
                                <g> </g>
                                <g> </g>
                                <g> </g>
                                <g> </g>
                                <g> </g>
                                <g> </g>
                                <g> </g>
                                <g> </g>
                                <g> </g>
                                <g> </g>
                                <g> </g>
                                <g> </g>
                                <g> </g>
                                <g> </g>
                            </svg></a>
                        <div class="px_search_inner">
                            <div class="px_search_widget">
                                <input type="text" class="form-control" placeholder="Search...">
                                <a href="#"><img src="public/images/svg/search.svg" alt=""></a>
                            </div>
                        </div>
                    </div>
                </div> -->
            </div>

        </div>
        <span class="px_body_overlay"></span>
    </div>
</div>
<?php /**PATH C:\xampp\htdocs\astroway-backend2\resources\views/v2/frontend/layout/header.blade.php ENDPATH**/ ?>